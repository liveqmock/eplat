...
public class Animator implements Runnable {
    ...
    private List<Blade>          grassList;
    private Group                treeContent;

    Animator(Group treeContent, List<Tree> treeList, List<Blade> grassList) {
    ...
    public void run() {
        for (Tree tree : treeList) {
            ...
            for (int i = 0; i < tree.generations.size(); i++) {
                ...
                branchGrowingAnimation.getChildren().add(animateBranchGrowing(branchGeneration, i, BRANCH_GROWING_DURATION)); // create animation
                                                                                                                              // for
                                                                                                                              // current crown
                treeWindAnimation.getChildren().add(animateTreeWind(branchGeneration, i, WIND_CYCLE_DURATION));
            }

            // Main animation: grass bending, tree bending, tree growing,
            // seasons
            // changing
    ...
    private Animation animateBranchGrowing(List<Branch> branchGeneration, int depth, Duration duration) {
        ...
        for (final Branch branch : branchGeneration) {
            Timeline branchGrowingAnimation = new Timeline(new KeyFrame(duration, new KeyValue(branch.base.endYProperty(), branch.length)));// line is growing by changinh endY
                                                                                                                                            // from 0 to brunch.length
            sameDepthBranchAnimation.getChildren().add(new SequentialTransition(
            // To set width from 0 to some value we use pause transition
            // with duration.one millisecond
            // trick to show lines
    ...
    private Animation animateTreeWind(List<Branch> branchGeneration, int depth, Duration duration) {
        ...
        for (final Branch brunch : branchGeneration) {
            ...
            wind.getChildren().add(
                TimelineBuilder.create().keyFrames(new KeyFrame(duration, new KeyValue(rotation.angleProperty(), depth * 2))).autoReverse(true).cycleCount(Animation.INDEFINITE).build());
        }
        return wind;
    }
    ...
    private Transition animateAutumn(List<Leaf> leafage, List<Blade> grass) {
        ...
        for (final Leaf leaf : leafage) {
            ...
            // disappear
            dissappearLeafage.getChildren().add(new SequentialTransition(fakeLeafageDown, FillTransitionBuilder.create().shape(leaf).toValue((Color) leaf.getFill()).duration(Duration.ONE).build()));
        }
        ...
        for (final Blade blade : grass) {
            // become yellow
            final FillTransition toYellow = FillTransitionBuilder.create().shape(blade).toValue(blade.AUTUMN_COLOR).delay(Duration.seconds(1 * Math.random())).duration(GRASS_BECOME_YELLOW_DURATION)
                .build();
    ...
    private Animation fakeFallDownEllipseAnimation(final Ellipse sourceEllipse, Color fakeColor, final HideMethod hideMethod) {
        ...
        Util.addChildToParent(treeContent, fake);

        PauseTransition replaceFakeWithSource = PauseTransitionBuilder.create().duration(Duration.ONE).delay(Duration.minutes(0.9 * Math.random() + 0.1)).onFinished(new EventHandler<ActionEvent>() {

            @Override
            public void handle(ActionEvent t) {

                final Point2D position = treeContent.sceneToLocal(sourceEllipse.localToScene(0, 0));
                ...
                DoubleBinding sinPath = new DoubleBinding() {
                    ...
                    protected double computeValue() {
                        return 50 * Math.sin((fake.translateYProperty().doubleValue() - position.getY()) / 20);
                    }
                };
                ...
                fake.rotateProperty().bind(fake.translateYProperty().multiply(2).add(Math.random() * 180));
        ...
        // fall down
            TranslateTransitionBuilder.create().duration(Duration.seconds(30)).toY(Math.random() * 30 + 1).node(fake).build(),
            // disappear
