    ...
    public Branch(Branch parentBranch, Type type, int depth) {
        ...
        switch (type) {
            case TOP: // creates top branch
                ...
                locAngle.set(RandomUtil.getRandom(10));
                break;
            case LEFT: // create left branch
            case RIGHT: // create right branch
                transY = parentBranch.length - RandomUtil.getGaussianRandom(0, parentBranch.length, parentBranch.length / 10, parentBranch.length / 10);
                locAngle.set(RandomUtil.getGaussianRandom(35, 10) * (Type.LEFT == type ? 1 : -1));
                if ((0 > globalAngle.get() || globalAngle.get() > 180) && depth < 4) {
                    length = parentBranch.length * RandomUtil.getGaussianRandom(0.3, 0.1); // branches
                    // pointed
                    // down
                    // are
                    // shorter
        ...
        getTransforms().add(new Rotate(locAngle.get(), 0, 0)); // rotate branch
                                                               // to local
                                                               // angle
                                                               // relative to
                                                               // parent branch
        globalH = getTranslateY() * Math.cos(Math.PI / 2 - parentBranch.globalAngle.get() * Math.PI / 180) + parentBranch.globalH;
        setBranchStyle(depth);
        Util.addChildToParent(parentBranch, this);
    }
