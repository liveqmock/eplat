...
public class OleControlPage extends TabPage {
    ...
    public void buildUI(Composite parent) {
        ...
        tabFolder.setSelectionBackground(
            new Color[] { ColorCache.getInstance().getColor(230, 233, 240), ColorCache.getInstance().getColor(157, 167, 195), ColorCache.getInstance().getColor(242, 244, 247), }, new int[] { 75 },
            true);
    ...
    private void createFlashItem(CTabFolder tabFolder) {
        ...
        Composite composite = WidgetUtil.getToolkit().createComposite(tabFolder);
    ...
    private void createFlashFileSelectedArea(Composite parent) {
        Composite fileSelectedArea = WidgetUtil.getToolkit().createComposite(parent);
        GridLayout layout = new GridLayout();
    ...
    private void createMediaPlayerFileSelectedArea(Composite parent) {
        ...
        layout.marginWidth = 0;
        WidgetUtil.createLabel(fileSelectedArea).setText("Select File:");
        mediaPlayerFileText = WidgetUtil.getToolkit().createText(fileSelectedArea, "");
    ...
    private void createFlashArea(Composite parent) {
        ...
        flash.addHookInterceptor(new OleHookInterceptor() {
            public boolean intercept(Msg message, int code, int wParam, int lParam) {
                if (message.getMessage() == Win32.WM_RBUTTONDOWN) {
                    Point cursor = flash.getControl().getParent().toControl(Display.getCurrent().getCursorLocation());
                    if (flash.getControl().getBounds().contains(cursor) && flash.getControl().isVisible()) {
        ...
        flash.getControl().getParent().addControlListener(new ControlAdapter() {
        ...
        flash.getControl().getParent().addPaintListener(new PaintListener() {
        ...
        try {
            InputStream is = this.getClass().getResourceAsStream("/flash.swf");
    ...
    private void createMediaPlayerArea(Composite parent) {
        final Label infoLabel = WidgetUtil.getToolkit().createLabel(parent, true);
        GridData gd = new GridData();
        gd.exclude = true;
        ...
        MediaPlayerEventListener listener = new MediaPlayerEventAdapter() {
            public void playStateChange(WMPMediaState newState) {
                if (newState == WMPMediaState.PLAYING) {
                    infoLabel.setText("Media Player Event Listener : Playing \"" + mediaPlayer.getCurrentMediaName() + "\"");
        ...
        mediaPlayer.addHookInterceptor(new OleHookInterceptor() {
            ...
            public boolean intercept(Msg message, int code, int wParam, int lParam) {
                if (message.getMessage() == Win32.WM_RBUTTONDOWN) {
                    Point cursor = mediaPlayer.getControl().getParent().toControl(Display.getCurrent().getCursorLocation());
                    if (mediaPlayer.getControl().getBounds().contains(cursor) && mediaPlayer.getControl().isVisible()) {
                        MessageBox messageBox = new MessageBox(container.getShell());
        ...
        mediaPlayer.getControl().getParent().addControlListener(new ControlAdapter() {
        ...
        mediaPlayer.getControl().getParent().addPaintListener(new PaintListener() {
    ...
    private void createTitle() {
        Label label = WidgetUtil.getToolkit().createLabel(container, "This page demonstrates how to use ole controls and observe ole events.", SWT.WRAP);
        GridData gd = new GridData();
        gd.widthHint = 300;
    ...
    private void resize(Control control) {
        ...
        width = width < control.getParent().getSize().x - 30 ? width : control.getParent().getSize().x - 30;
        gd.widthHint = width;
